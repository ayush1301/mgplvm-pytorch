...
#### load data ####
Y = load_data(name = name, region = region, binsize = binsize, subsamp = False, behavior = False,
              shift = False, thresh = 2, shiftsize = 0)
...
def cb_ard(mod, i, loss):
    ...
    if i % 400 == 0:
        ...
        args = np.argsort(ls)
        print(region, np.round(ls[args], 2), '', np.round(ss[args], 2), '', np.round(ms[args], 2), np.round(time.time()-t0))
        t0 = time.time()
    return False
...
for nrep in range(nreps):
    for ifold in range(nfold):
        ...
        N1 = np.concatenate([folds[i] for i in range(nfold) if i != ifold]) # train neurons

        Ytrain = Y[..., T1] # extract training time points
        print('\n', Y.shape, np.var(Y[0, N2, :][:, T2]), len(N1), len(T1), len(N2), len(T2))

        print('nrep:', nrep, ' ifold:', ifold, len(N1), len(N2))

        iter_ = iter([ds_fit[-1]]) if model == 'bgpfa_ard' else iter(ds_fit) # if using bGPFA with ARD, only fit largest dimensionality
        for dnum, d_fit in enumerate(iter_): # for each dimensinoality
            ...
            if model == 'fa': #Poisson FA
                ...
                # instantiate model
                mod = mgp.models.Lvgplvm(n, T, d_fit, n_samples, lat_dist, lprior, lik, Y = Ytrain, Bayesian = False, rel_scale = rel_scale*0.1).to(device)

            elif model in ['gpfa', 'bgpfa', 'bgpfa_ard', 'gplvm']: # some version of GPFA
            ...
            if model == 'fa':
                ...
                # perform crossvalidated training using bGPFA library
                mod, split = mgp.crossval.train_cv_bgpfa(Y, device, train_ps, fit_ts, d_fit, 8,
                                                        T1=T1, N1=N1, test=False, rel_scale = rel_scale,
                                                        likelihood = 'Poisson', model = 'bgpfa',
            ...
            m = T
            T2cv, N2cv = not_in(np.arange(m), T1cv), not_in(np.arange(n), N1cv) # these are our training timepoints and neurons
            ...
            for _ in range(100): # number of runs for memory reasons
                ...
                svgp_elbo, kl = mod(data, 10, m=T, analytic_kl=(False if model == 'fa' else True), neuron_idxs = N1)
                newLLs.append((svgp_elbo-kl).item()/np.prod(Y.shape)*n/len(N1)) # compute importance weighted log likelihoods
            ...
            for _ in range(100):
                ...
                svgp_elbo, kl = mod(data[:, :, T2], 10,
                                    batch_idxs=T2, neuron_idxs=N2, m=len(T2))
                newMargs.append((svgp_elbo-kl).item()/(len(T2) * len(N2) * n_samples))
            LL_marg[nrep, dnum, ifold] = np.mean(newMargs) # store result

            # print some progress
            print(d_fit, 'LL:', np.round(LL, 4), 'MSE:', np.round(MSE, 4), 'marg LL:', np.round(LL_marg[nrep, dnum, ifold], 4))

save = True

if save:
    pickle.dump([ds_fit, LLs, LL_marg, MSEs, LL_trains], open(savename_cv+'.pickled', 'wb'))
